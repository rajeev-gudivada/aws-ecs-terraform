...
/* Private subnet - Persistence*/
resource "aws_subnet" "persistence_private_subnet" {
  vpc_id                  = aws_vpc.vpc.id
  count                   = length(var.persistence_private_subnets_cidr)
  cidr_block              = element(var.persistence_private_subnets_cidr, count.index)
  availability_zone       = element(var.availability_zones, count.index)
  map_public_ip_on_launch = false
  tags = {
...
resource "aws_route_table_association" "persistence_private" {
  count          = length(var.persistence_private_subnets_cidr)
  subnet_id      = element(aws_subnet.persistence_private_subnet.*.id, count.index)
  route_table_id = aws_route_table.private.id
}
/*==== VPC's Default Security Group ======*/
resource "aws_security_group" "default" {
  name        = "vpc-om-${var.environment}-default-sg"
